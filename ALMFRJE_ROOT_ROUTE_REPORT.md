...
الاستشارات على `/con`، مراح على `/mrahi`، الملاحظات على `/notes`)، فتُرك الجذر فارغاً عمداً
...
- لم تُلمَس مسارات `/con`,`/mrahi`,`/notes` ولا أي API.
...
- **`prebuild` (حارس الحدود `scripts/check-boundaries.mjs`):** اجتاز ✅ —
  «✅ حدود المشاريع سليمة: لا تداخل بين almfrji / con / mrahi / notes».
...
## 8) تأكيد عدم لمس Alaoufi.me أو marahi

تم. **لم يُلمَس مستودع `alaoufi/Alaoufi.me` ولا `alaoufi/marahi`** — لا تعديل ولا commit ولا
