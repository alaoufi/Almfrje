...
function pPub(){
  ...
  pubs.forEach(function(p){
    ...
    if(isPdf&&p.file_path) h+='<button class="btn bs bsm" onclick="if(typeof openPdfViewer===\'function\')openPdfViewer(\''+p.file_path.replace(/'/g,"\\'")+'\',\''+(p.title||'').replace(/\\/g,'\\\\').replace(/\'/g,"\\\'")+'\',{description:\''+(p.description||'').replace(/\\/g,'\\\\').replace(/\'/g,"\\\'")+'\'});else window.open(\''+p.file_path+'\')">👁️ معاينة</button>';
...
async function svPub(){
  ...
  if(type==='text'){
    ...
    // PDF — ارفع الملف أولاً إلى /api/upload للحصول على URL ثم احفظ النشرة
    var fileEl=$g('pub-file');
    var file=fileEl&&fileEl.files&&fileEl.files[0];
    if(!file){toast('اختر ملف PDF','er');return;}
    ld(1);
    ...
    try{
      ...
      upR=await res.json();
    }catch(e){upR={error:'خطأ شبكة'};}
    if(!upR||!upR.ok){
      ld(0);
      toast((upR&&upR.error)||'فشل رفع الملف','er');
      return;
    }
    body.file_path=upR.url;
    body.file_name=upR.filename;
  }
