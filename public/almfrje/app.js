...
const HINTS = {
  ...
  hierarchy: ['العرض الهرمي', 'عرض منظّم للشجرة على شكل بطاقات متدرّجة بالألوان حسب الجيل، مع صورة كل فرد قبل اسمه.\n• «توسيع الكل» يفتح كل الفروع.\n• «طباعة / PDF» يصدّر الشجرة كاملة.\n• الضغط على الصورة أو الاسم يفتح العدسة السريعة.'],
    ...
    listEl.innerHTML = keys.map(g => {
      ...
      const cards = arr.map(p => {
        ...
        return `<div class="tl-card" data-lensid="${p.id}">
          <span class="tl-av">${avatar(p)}</span>
          <div class="tl-body">
...
function screenDescendants(arg) {
  ...
  const indexRows = idx.map(({ p: x, num, depth }) => {
    ...
    return `<div class="idx-row${isRoot ? ' idx-root' : ''}${isParent ? ' idx-parent' : ''}" style="padding-inline-start:${pad}px">
      ...
      <span class="idx-av">${avatar(x)}</span>
...
function hierNode(id, depth) {
  ...
  let html = `<li class="hier-li lvl${lvl}">
    <div class="hier-row">
      ${toggle}
      <div class="hier-main">
        <div class="hier-line"><span class="hier-av" data-hopen="${id}">${avatar(p)}</span><span class="hier-name ${nameCls(p)}" data-hopen="${id}"${nameTitle(p)}>${esc(p.name)}</span>${nickSuffix(p)}${statusTag(p)}</div>
        ${meta}
      </div>
    </div>`;
  if (has && open) {
...
const GUIDE = [
  ...
  { sec: '🌳 العرض والتصفّح', items: [
    ...
    { t: 'العرض الهرمي العام', fn: 'عرض منظّم للشجرة بالبطاقات مع صور الأفراد.', brief: 'بطاقات متدرّجة بالألوان حسب الجيل، وصورة كل فرد قبل اسمه.', det: 'تظهر صورة كل فرد (أو أيقونة لمن بلا صورة) قبل اسمه. «توسيع الكل» يفتح كل الفروع، و«طباعة/PDF» يصدّر الشجرة كاملة، والضغط على الصورة أو الاسم يفتح العدسة السريعة.' },
