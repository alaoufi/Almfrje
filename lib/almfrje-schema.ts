...
export const ALMFRJE_SCHEMA_SQL = `
      ...
      -- مكتبة موسّعة: تصنيف العنصر (فارغ=صورة/وثيقة حسب kind، أو قصيدة له/فيه)، خصوصية العرض،
      -- ونصّ للقصائد. is_public=true افتراضياً (توافقاً مع العناصر القديمة — كانت تُعرض للجميع).
      ...
      -- رؤية عنصر المكتبة: العامّ يراه كل عضو؛ والمخفيّ لا يراه إلا الإدارة أو صاحبُه أو ذريّتُه.
      ...
      -- الوثائق
      DROP POLICY IF EXISTS docs_sel ON public.almfrje_documents; CREATE POLICY docs_sel ON public.almfrje_documents FOR SELECT USING (public.almfrje_can_see_doc(person_id, is_public));
      ...
      INSERT INTO public.almfrje_settings (key, value)
        VALUES ('schema_rev', '"2026-08-23-1"'::jsonb)
