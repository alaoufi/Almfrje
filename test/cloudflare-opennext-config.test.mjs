...
test('traces the Cloudflare PostgreSQL runtime for database setup routes', () => {
  const includes = nextConfig.outputFileTracingIncludes;
  const pgCloudflareFiles = './node_modules/pg-cloudflare/dist/**/*';

  assert.ok(includes['/api/almfrje-setup']?.includes(pgCloudflareFiles));
  assert.ok(includes['/api/setup']?.includes(pgCloudflareFiles));
});
